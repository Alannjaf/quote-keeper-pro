...
export function QuotationDocuments({
  ...
  const fetchDocuments = async () => {
    const {
      data,
      error
    } = await supabase.from('vendor_documents').select('*').eq('quotation_id', quotationId).order('created_at', {
      ascending: false
    });
    if (error) {
      toast({
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="relative">
          <input type="file" multiple onChange={handleFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" />
      ...
      <div className="border rounded-lg divide-y">
        {documents.length === 0 ? <div className="p-4 text-center text-gray-500">
            No documents uploaded yet
          </div> : documents.map(doc => <div key={doc.id} className="p-4 flex items-center justify-between bg-gray-900 hover:bg-gray-800">
              ...
              <div className="flex items-center space-x-2">
                <Button variant="ghost" size="sm" onClick={() => handleDownload(doc.file_path, doc.file_name)}>
                  <Download className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleDelete(doc.id, doc.file_path)}>
